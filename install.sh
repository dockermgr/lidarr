...
# @@File             :  install.sh
# @@Description      :  Container installer script for lidarr
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/lidarr"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="lidarr"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/lidarr"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/lidarr"
...
HUB_IMAGE_URL="casjaysdevdocker/lidarr"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [lidarr.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
CONTAINER_ADD_CUSTOM_PORT="0.0.0.0:8686:8686"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_MOUNTS="/mnt/music:/music,/mnt/downloads:/downloads"
CONTAINER_MOUNTS+=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
CONTAINER_HOSTNAME="${CONTAINER_HOSTNAME:-${APPNAME:-lidarr}}"
...
if [ -n "$LIDARR_USERNAME" ]; then
  CONTAINER_USER_NAME="$LIDARR_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${LIDARR_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$LIDARR_PASSWORD" ]; then
  CONTAINER_USER_PASS="$LIDARR_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${LIDARR_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
